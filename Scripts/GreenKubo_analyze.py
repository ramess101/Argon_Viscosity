...
#guess_default = [0.94404798,    0.1525724 ,  129.11150109,   10.95179724]
...
#            guess = guess_default
        ...
        f = open('opt_fit_rho'+str(self.irho),'w')

        for param in opt_fit:
...
#        np.savetxt('GK_eta_inf',eta_inf,fmt='%0.7f')
    ...
    def fit_eta(self,t_data,eta_data,w8_data,tcut=tcut_default,tlow=tlow_default,guess=guess_default):
        ...
        if np.all(guess == guess_default): #Just a simple way to check that we are fitting to the actual data

            #Thin data to speed up fit
            eta_data = eta_data[::100]
            w8_data = w8_data[::100]
            t_data = t_data[::100]
        ...
        if np.all(guess == guess_default):

            SSE_opt = 1e10
            
            for iguess in range(10):
                ...
                opt_fit_i = opt.x
                eta_inf_i = self.calc_eta_estimate(opt_fit_i)
                ...
                print(' the optimal was: '+str(opt_fit_i))
                print(' with SSE of: '+str(SSE_i))
                print(' and eta_inf: '+str(eta_inf_i))
